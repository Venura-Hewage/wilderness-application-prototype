...
export default class APIGen {
  // The constructor runs automatically when you create a new instance with: new PixabayAPI()
  //? It stores the API key and URL from CONFIG so every method can access them via "this".
  constructor() {
    // Member: stores the API key used to authenticate every request sent to Pixabay.
    this.apiKey = CONFIG.apiKey;
    // Member: stores the base URL that all API requests are sent to.
    this.apiUrl = CONFIG.apiUrl;
    // Member: store ths base URL that all API requests for Pixabay Videos are sent to.
  }
